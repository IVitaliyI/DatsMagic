...
class HTTPClient:
    ...
    async def _get(self, session: aiohttp.ClientSession, endpoint: str, params: Optional[dict] = None):
        url = self.base_url + endpoint
        async with session.get(url, params=params) as response:
            response.raise_for_status()
            return await response.json()
    
    async def _post(self, session: aiohttp.ClientSession, endpoint: str, data: Optional[dict] = None):
        url = self.base_url + endpoint
        async with session.post(url, json=data) as response:
            response.raise_for_status()
            return await response.json()
    
    async def request(self, method: str, endpoint: str, params: Optional[dict] = None, data: Optional[dict] = None):
    ...
    async def bulk_request(self, requests: list):
        """
        Асинхронное выполнение нескольких запросов (GET или POST).
        requests: список запросов вида [{"method": "GET", "endpoint": "/path", "params": {...}}, {...}]
        """
        tasks = []
        async with aiohttp.ClientSession(headers=self.headers) as session:
            for req in requests:
...
#         {"method": "POST", "endpoint": "/update", "data": {"id": 1, "value": "new"}}
#     ]
